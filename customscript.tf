...
resource "azurerm_storage_account" "thefinalhawk9812739" {
...
resource "azurerm_storage_container" "data" {
  ...
  depends_on=[
    azurerm_storage_account.thefinalhawk9812739
    ]
}
...
resource "azurerm_virtual_machine_extension" "vmextension" {
  ...
  settings = <<SETTINGS
    {
        "fileUris": ["https://${azurerm_storage_account.thefinalhawk9812739.name}.blob.core.windows.net/data/IIS_Config.ps1"],
